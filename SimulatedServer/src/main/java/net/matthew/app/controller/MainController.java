	 ...
	 final Gson gson = new GsonBuilder().setPrettyPrinting().create();
      ...
      String getLatest(HttpServletRequest request,HttpServletResponse response) {
    	  ...
    	  if("a".equals(productCode)) {
    		  Block productCaptialBlock = getGetValue(Static_Value.HTTP+Static_Value.ABS_A_CAPITAL_CHAIN_DOMAIN,Static_Value.ABS_A_CAPITAL_CHAIN_PORT,Static_Value.ABS_GET_LAST_METHOD);
    		  Block productReputationBlock = getGetValue(Static_Value.HTTP+Static_Value.ABS_A_REPUTATION_CHAIN_DOMAIN,Static_Value.ABS_A_REPUTATIONL_CHAIN_PORT,Static_Value.ABS_GET_LAST_METHOD);
    		  Block valueBlock = getGetValue(Static_Value.HTTP+Static_Value.ABS_A_VALUE_CHAIN_DOMAIN,Static_Value.ABS_A_VALUE_CHAIN_PORT,Static_Value.ABS_GET_LAST_METHOD);
      ...
      Block getGetValue(String url, int port,String method) {
      	...
      	try {
  			...
  			if(returnBlock.getVac().contains("illegalContent")) {
  				...
  				returnBlock=getGetValue(url, port,  Static_Value.ABS_GET_BY_INDEX_METHOD+"/"+(returnBlock.getIndex()-1));
  				
  			}
  			if(illegalContent) {
  				returnBlock.setComments("The content you visit is has illegalContent in the latestblock, so we show older one to you"
  						+ "please contact Admin to fix it, the index is "+index+", and the hash code is "+hash);
  			}
  			return returnBlock;
